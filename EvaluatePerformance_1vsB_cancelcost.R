...
source("Cumulative_cancelcost.R")

EvaluatePerformance_1vsB_cancelcost <- function(matrix,Factors,PerformanceMetric,method,percentage,filename,default_bins){
  ...
  Cumulative_cancelcost(matrix,Factors,PerformanceMetric,method,template)
  ...
  for(factor in Factors){
    ...
    #writeWorksheetToFile(template,filename,sheet=factor,styleAction = XLC$STYLE_ACTION.NONE,startRow = 1, startCol = 1,header=F)
    ...
    if(factor=="PILOT_SECURITY"){
      ...
      dataset <- matrix %>% filter(!is.na(rlang::UQ(factor_quo)))
    }
    ...
    if (all_obs>0){
      ...
      if(method=="straight"){
        test_result <- dataset %>% group_by(!!bin_factor_quo) %>%
          summarise(pvalue = ifelse(length(which(!is.na(rlang::UQ(PerformanceMetric))))>1, 1-pnorm(t.test(rlang::UQ(PerformanceMetric),mu=benchmark)$statistic),NA),
      ...
      #method="weighted"
        ...
        test_result <- dataset %>% group_by(!!bin_factor_quo) %>%
          summarise(wt_avg1 = wt.mean(rlang::UQ(PerformanceMetric)[which(!is.na(rlang::UQ(PerformanceMetric)))],notional[which(!is.na(rlang::UQ(PerformanceMetric)))]),
                    ...
                    pvalue = ifelse(length(!is.na(rlang::UQ(PerformanceMetric)))>1,1-pnorm((wt_avg1-benchmark)/std_error1),NA),
        ...
        test_result <- dataset %>% group_by(!!bin_factor_quo) %>%
          summarise(df = n(),
                    pvalue = ifelse(length(which(!is.na(rlang::UQ(PerformanceMetric))))>1, 1-pnorm((wilcox.test(rlang::UQ(PerformanceMetric),rep(0,df))$statistic-df*df/2)/sqrt(df*df*(2*df+1)/12)),NA),

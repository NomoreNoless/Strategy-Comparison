Cumulative_cancelcost <- function(matrix,Factors,PerformanceMetric,method,template){
  ...
  if(method=="straight"){
    test_result <- data.frame(pvalue = 1-pnorm(t.test(matrix[,PerformanceMetric],mu=benchmark)$statistic),
    ...
    test_result <- data.frame(average=wt.mean(matrix[,PerformanceMetric],notional),
                              df = sum(notional[which(!is.na(notional))])**2/sum(notional[which(!is.na(notional))]**2),
                              std_error = wt.sd(matrix[,PerformanceMetric],matrix[,"notional"])/sqrt(df),
                              pvalue = 1-pnorm((average-benchmark)/std_error),
    ...
    test_result <- data.frame(df=length(matrix[,PerformanceMetric]),
                              pvalue = 1-pnorm((wilcox.test(matrix[,PerformanceMetric],rep(0,df))$statistic-df*df/2)/sqrt(df*df*(2*df+1)/12)),
